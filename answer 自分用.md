...
ログイン後 X window を起動し (大抵自動で起動する)、以下を順に行え。
  ...
  * [ ] 4. `yes` をたくさん起動。その後、コマンド名指定で kill したいので、`kill yes` で強制終了
  * [ ] 5. ターミナルを閉じる
  * [ ] 6. 特になし
# 2&emsp;UNIX コマンド・基本アプリケーション
  ...
  * [ ] ファイルの各行から指定した一部分を標準出力する cut コマンドを使いたくなるが、行指定はできないので使わない。  
        ここでは行指定のできる sed コマンドを使って `sed -n 2p men.csv` とする。
